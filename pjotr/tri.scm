...
(def (min-xs ts) (reduce min +inf0 (map (compose min-x cdr) ts)))
(def (max-xs ts) (reduce max -inf0 (map (compose max-x cdr) ts)))
(def (min-ys ts) (reduce min +inf0 (map (compose min-y cdr) ts)))
(def (max-ys ts) (reduce max -inf0 (map (compose max-y cdr) ts)))
...
(def (tri->div ax ay . t) (call/cc (lambda (k) `(<DIV> style: ,(string-append
  "width:0;"
  "height:0;"
  "position:absolute;"
  ...
  (apply (fn
  || GE: points ... => (k "") ; hide non-representable triangles
...
(def (decompose t) ; and tag
   (cases t || [((x1 y1) as p1) ((x2 y2) as p2) ((x3 y3) as p3)] =>
      (let ([dx1 (abs (- x2 x1))]
	    ...
	    [dy3 (abs (- y3 y1))])
      (cond ; too small
	 ((and (< dx1 0.001) (< dx2 0.001) (< dx3 0.001)
	       (< dy1 0.001) (< dy2 0.001) (< dy3 0.001))
	  (list (cons GE: t)))
	 ; top
